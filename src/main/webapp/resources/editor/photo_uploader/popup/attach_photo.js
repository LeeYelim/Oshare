		...
		try{
			if( !oNavigator.ie ){
				if(!!oNavigator.safari && oNavigator.version <= 5){
					...
					/**
					 *  단일 파일 업로드 : bSupportDragAndDropAPI = false;
					 *  다중 파일 업로드 : bSupportDragAndDropAPI = true;
					 */
    ...
    function html5Upload() {	
    	...
    	sUploadURL= 'http://localhost:8000/controller/editor/multiplePhotoUpload'; 	//upload URL
	...
	function onAjaxError (){
 	...
 	function callFileUploader (){
 		oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"),{
 			sUrl  : '/controller/editor/photoUpload',	//파일업로드시 동작되는 컨트롤러 url
